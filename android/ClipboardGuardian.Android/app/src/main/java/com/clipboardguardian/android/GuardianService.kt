...
class GuardianService : Service() {
    ...
    private fun handleClipboardMutation() {
        try {
            ...
            pendingRequest = PendingRequest(requestId, RequestType.WRITE, text)

            Log.i(tag, "Clipboard changed; sample=${text?.take(80)}")
            logWriter.log("copy", "pending", text, "ожидание решения пользователя")

            // временно очищаем буфер пока ждем подтверждение
            neutralizeClipboard()

            startActivity(
                ApprovalActivity.newIntent(this, requestId, text).apply {
    ...
    private fun handleReadRequest(requestId: String) {
        try {
            val clip = clipboardManager.primaryClip
            val text = clip?.takeIf { it.itemCount > 0 }
                ?.getItemAt(0)
            ...
            pendingRequest = PendingRequest(requestId, RequestType.READ, text)
            logWriter.log("paste", "pending", text, "ожидание решения пользователя (READ)")

            startActivity(
                ApprovalActivity.newReadIntent(this, requestId, text).apply {
                    addFlags(Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_CLEAR_TOP)
                }
            )
        } catch (t: Throwable) {
    ...
    private fun handleDecision(decision: String, requestId: String) {
        val request = pendingRequest ?: return
        if (!TextUtils.equals(request.id, requestId)) {
            return
        }
        pendingRequest = null

        when (request.type) {
            RequestType.WRITE -> handleWriteDecision(decision, request)
            RequestType.READ -> handleReadDecision(decision, request)
        }
    }

    private fun handleWriteDecision(decision: String, request: PendingRequest) {
        when (decision) {
            DECISION_ALLOW -> {
                if (!request.sample.isNullOrEmpty()) {
                    setClipboardSilently(request.sample)
                    lastApprovedText = request.sample
                }
                logWriter.log("copy", "allowed", request.sample, "пользователь разрешил")
            }
            DECISION_DENY -> {
                ...
                logWriter.log("copy", "blocked", request.sample, "пользователь запретил")
            }
        }
    }

    private fun handleReadDecision(decision: String, request: PendingRequest) {
        when (decision) {
            DECISION_ALLOW -> {
                sendBroadcast(
                    Intent(ACTION_READ_RESULT).apply {
                        ...
                        putExtra(EXTRA_SAMPLE, request.sample)
                        putExtra(EXTRA_DECISION, DECISION_ALLOW)
                    }
                )
                logWriter.log("paste", "allowed", request.sample, "пользователь разрешил (READ)")
            }
            DECISION_DENY -> {
                sendBroadcast(
                    Intent(ACTION_READ_RESULT).apply {
                ...
                logWriter.log("paste", "blocked", request.sample, "пользователь запретил (READ)")
            }
        }
    }
    ...
    private fun setClipboardSilently(text: String?) {
        ignoreChangesUntil = SystemClock.elapsedRealtime() + 750
